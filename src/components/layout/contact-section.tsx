        ...
        return (
          <div className="space-y-4">
            {Object.entries(formData.services).map(([service, isSelected]) => (
              <label 
                key={service} 
                className={`flex items-center gap-3 px-4 py-3 rounded-lg border cursor-pointer transition-colors ${isSelected ? 'bg-primary/5 border-primary' : 'hover:bg-accent'}`}
              >
                <div className="relative flex items-center">
                  <input
                    ...
                    className="w-5 h-5 border-2 rounded accent-primary focus:ring-primary cursor-pointer"
                  />
                </div>
                <span className="font-medium">
                  {service === 'gevelisolatie' && 'Gevelisolatie'}
                  {service === 'dakisolatie' && 'Dakisolatie'}
                  {service === 'vloerisolatie' && 'Vloerisolatie'}
                </span>
              </label>
            ))}
            {errors.services && (
              <p className="text-destructive text-sm mt-1">{errors.services}</p>
            )}
          </div>
        );
